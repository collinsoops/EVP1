...
<form id="msform" name="joe" action="preview.php" method="POST" onSubmit="return sapatka()">
	...
	<fieldset>
		...
		$results = $conn->prepare("SELECT * FROM candidates WHERE position_id= :a");
			...
			for($i=0; $rows = $results->fetch(); $i++){
				?>
				<option style="padding: 35px 50px 35px 80px; background:url('admin/candidates/<?php echo $rows['image']; ?>')  no-repeat scroll 5px 7px / 70px auto rgba(0, 0, 0, 0);" value="<?php echo $rows['firstname'] ?>"><?php echo $rows['firstname'] ?> - <?php echo $rows['lastname'] ?></option>
				<?php
			}
		?>
		</select>
	...
	if(($exrxrxrx!='chairman') AND ($exrxrxrx!='Senator') AND ($exrxrxrx!='Representative')){
	?>
	<fieldset>
		<h2 class="fs-title"><?php echo $rowasa['description'] ?></h2>
		<h3 class="fs-subtitle">Pls. Select Only One</h3>
	...
	<fieldset>
		...
		<?php
		$dsds=10;
		$results = $conn->prepare("SELECT * FROM candidates WHERE position= :a");
			$results->bindParam(':a', $dsds);
			$results->execute();
			for($i=0; $rows = $results->fetch(); $i++){
		?>
